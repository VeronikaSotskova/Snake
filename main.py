...
from gui import Gui, print_text
...
def game_cycle():
    while control.run:
        control.cur_t = time.time() - gui.start_time
        for event in pygame.event.get():
            if event.type == QUIT:
                control.run = False
                pygame.quit()
        ...
        if gui.game == "GAME":
            ...
            '''Отрисовывает текущее время с момента начала игры (в секундах)'''
            #print_text("Current second: %d" % control.cur_t, 220, 5, pygame.Color("Red"), 'monaco', 24, window)
